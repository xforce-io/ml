...
class PrioritizedReplayBuffer:
    ...
    def _initialize_buffers(self, state):
        ...
        print(f"PrioritizedReplayBuffer在设备{self.device}上初始化完成，状态形状: {state_shape}")
    ...
    def sample(self, batch_size):
        ...
        for i in range(batch_size):
            ...
            a = segment * i
            b = segment * (i + 1)
            v = a + torch.rand(1, device=self.device).item() * (b - a)
            
            # 从sum_tree获取样本
            _, priority, data_idx = self.sum_tree.get_leaf(v)
        ...
        # 权重 w_j = (1/N * 1/P(j))^β
        sampling_probabilities = batch_priorities / self.sum_tree.total_priority()
